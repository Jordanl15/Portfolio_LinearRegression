...
def plot_data_and_regression(data1, a, b, ask_plot, ask_line, x_value):
    if ask_plot == 1:
        if ask_line == 1 and x_value != '':  # Check if ask_line is checked and x_value is not empty
            try:
                x_value = int(x_value)  # Convert x_value to integer
                y1 = a + b * x_value
